...
import NftManager from "../api/nft-manager";
...
router.route('/nft/transfer').post(
    async (request, response, next) => {
        try {
            const { wallet, destinationAddress, nftAddress, amount } = request.body;

            console.log(`[${new Date().toLocaleString()}] => Received NFT Transfer Call => Transfering Nft: ${nftAddress} => QTY ${amount} => FROM WALLET: ${wallet.address} => TO WALLET: ${destinationAddress}`);

            response.status(200).send(await NftManager.instance.transferNftToWallet(wallet, destinationAddress, nftAddress, amount));

            console.log(`[${new Date().toLocaleString()}] => Received NFT Transfer Call => Transfering Nft: ${nftAddress} => QTY ${amount} => FROM WALLET: ${wallet.address} => TO WALLET: ${destinationAddress} => COMPLETE`);
        } catch (e) {
            console.log({ status: 'exceptionHandled', error: e });
            response.status(200).send({ success: false });
        }
